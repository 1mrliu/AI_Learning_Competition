...
# 将文件转化为数字数组
# for  c in text:
#    print(c)  c 代表的是文章中使用到的字母和符号
encoded = np.array([vocab_to_int[c] for c in text], dtype=np.int32)
print("_____________________________")
print(encoded) # 输出的是文章中文字代表的字母
...
def get_batches(arr,n_seqs,n_steps):
    ...
    for n in range(0,arr.shape[1],n_steps):
        ...
        y[:,:-1], y[:,-1] = x[:,1:], x[:,0]
        yield x, y
# 上边的代码定义了一个generator，调用函数会返回一个generator对象，可以获取一个batch
# bath大小为50  batch个数是10
...
def build_inputs(num_seqs,num_steps):
    '''
    num_seqs:每个batch中的序列个数 64
    num_steps:每个序列包含的字符数 50
    '''
    # inputs 
