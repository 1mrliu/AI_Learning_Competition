...
if __name__ == "__main__":
    ...
    data = deal_user_star_level(data)


    if online == False:
        # train, test = train_test_split(data, train_size= 0.7, test_size= 0.3, random_state=3)
        train = data.loc[data.day < 23]
    ...
    if online == False:
        print ('--------Start XGBoost_Model-------')
        XGB =  xgb.XGBClassifier(learning_rate = 0.03, n_estimators= 200,
                                 max_depth= 9, min_child_weight= 5,
                                 subsample=0.5, colsample_bytree=0.8,
                                 booster='gbtree',
                                 objective= 'binary:logistic', scale_pos_weight=1)
        XGB.fit(train[features].as_matrix(), train[target].as_matrix())
        test['xgb_predict'] = XGB.predict(test[features].as_matrix())
        print ('XGBoost出现的误差是：', log_loss (test[target].as_matrix(), test['xgb_predict']))
        # 出现的误差是： 0.668940888   0.64054873462  0.58228

        print ('--------Start LGB_Model-------')
        clf = lgb.LGBMClassifier(num_leaves=70, max_depth=8, n_estimators=90, n_jobs=20)
        ...
        # 出现的误差是： 0.08830365872348662  0.08258136034739007
    else:
